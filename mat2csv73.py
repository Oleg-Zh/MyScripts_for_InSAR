...
for inputfile in currentDirectory.glob(currentPattern):
    ...
    ts = h5py.File(inputfile)
    def from_mat_date(md):
        return str(date.fromordinal(int(md)) + timedelta(days=md%1) - timedelta(days = 366))

    columns_date = [(lambda i: from_mat_date(int(i)))(i) for i in list(ts['day'][0])]

    x = np.array(list(ts['ph_mm'])).T
    df = pd.DataFrame(x,columns=columns_date)
    dstat = pd.DataFrame()
    ...
    x = np.array(list(ts['lonlat'])).T
    d1 = pd.DataFrame(x, columns=columns)
